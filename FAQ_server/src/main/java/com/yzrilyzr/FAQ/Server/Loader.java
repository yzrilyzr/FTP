	...
	private void init()
	{
		try
		{
			if(Server==null)
			{
				...
				if(!u.exists())
				{
					onPrint("找不到内核文件:"+u.getAbsolutePath());
					InputStream is=ctx.getAssets().open("server.dat");
					BufferedOutputStream fos=new BufferedOutputStream(new FileOutputStream(u.getAbsolutePath()));
					byte[] b=new byte[8192];
					int i=0;
					while((i=is.read(b))!=-1)fos.write(b,0,i);
					is.close();
					fos.flush();
					fos.close();
				}
